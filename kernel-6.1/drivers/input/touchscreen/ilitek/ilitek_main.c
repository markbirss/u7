...
static void ilitek_esd_check(struct work_struct *work)
{
	...
	if (ts->operation_protection || ts->esd_skip) {
		TP_MSG(NULL,
		       "[ESD] operation_protection: %hhu, esd_skip: %hhu\n",
	...
	for (; retry-- > 0;) {
		...
		if (protocol_ver != ts->dev->protocol.ver) {
			TP_ERR(NULL,
			       "unexpected ptl ver (referance)%x vs. %x\n",
			       protocol_ver, ts->dev->protocol.ver);
			continue;
		}
	...
	for (i = 0; i < cnt; i++) {
		...
		while (count-- >= 0) {
			...
			if (msgs[i].flags == I2C_M_RD)
				err = ilitek_dma_i2c_read(msgs[i].buf,
							  msgs[i].len);
			else if (!msgs[i].flags)
				err = ilitek_dma_i2c_write(msgs[i].buf,
							   msgs[i].len);

			if (err < 0) {
				TP_ERR(NULL,
				       "i2c[0x%hx] dma tx/rx failed, err: %d\n",
				       msgs[i].addr, err);
				mdelay(20);
				continue;
			}
						    ...
						    int r_len)
{
	...
	struct i2c_client *client = (struct i2c_client *)ts->client;
	struct i2c_msg msgs[2] = { { .addr = addr,
				     .flags = 0,
				     .len = w_len,
				     .buf = cmd,
				     SCL_RATE(400000) },
	...
	if (w_len > 0 && r_len > 0 && !delay_ms) {
		if (ilitek_i2c_transfer(msgs, 2) < 0) {
			...
			addr = (addr == ILITEK_BL_ADDR) ? client->addr :
							  ILITEK_BL_ADDR;
...
static int ilitek_request_input_dev(void)
{
	...
	x_min = 0;
	y_min = 0;
	x_max = TOUCH_SCREEN_X_MAX;
	y_max = TOUCH_SCREEN_Y_MAX;
#endif
...
static int ilitek_touch_down(int id, int x, int y, int p, int h, int w)
{
	...
	if (tpd_dts_data.use_tpd_button) {
		if (FACTORY_BOOT == get_boot_mode() ||
			...
			tpd_button(x, y, 1);
			TP_DBG(NULL,
			       "tpd_button(x, y, 1) = tpd_button(%d, %d, 1)\n",
			       x, y);
		}
	}
#endif
...
static int ilitek_touch_release(int id)
{
	...
	if (tpd_dts_data.use_tpd_button) {
		if (FACTORY_BOOT == get_boot_mode() ||
			...
			tpd_button(0, 0, 0);
			TP_DBG(NULL,
			       "tpd_button(x, y, 0) = tpd_button(0, 0, 0)\n");
		}
	}
#endif
#endif
	...
	for (j = 0; j < ts->dev->tp_info.key_num; j++) {
		if ((x >= ts->dev->key.info.keys[j].x &&
			...
			if (tpd_dts_data.use_tpd_button) {
				...
				TP_DBG(NULL,
				       "key index=%x, tpd_dts_data.tpd_key_local[%d]=%d key down\n",
			...
			ts->is_touched = true;
			TP_DBG(NULL,
			       "Key, Keydown ID=%d, X=%d, Y=%d, key_status=%d\n",
	...
	for (j = 0; j < ts->dev->tp_info.key_num; j++) {
		...
		if (check_point) {
			if (x < ts->dev->key.info.keys[j].x ||
...
#if ILITEK_PLAT != ILITEK_PLAT_MTK
				input_report_key(ts->input_dev,
						 ts->dev->key.info.keys[j].id,
						 0);
#else
#ifndef MTK_UNDTS
				if (tpd_dts_data.use_tpd_button) {
					TP_DBG(NULL,
					       "key index=%x, tpd_dts_data.tpd_key_local[%d]=%d key up\n",
					       j, j,
					       tpd_dts_data.tpd_key_local[j]);
					ilitek_touch_release(0);
				}
#else
				ilitek_touch_release(0);
				...
				TP_DBG(NULL,
				       "Key, Keyout ID=%d, X=%d, Y=%d, key_status=%d\n",
				       ts->dev->key.info.keys[j].id, x, y,
				       ts->dev->key.clicked[j]);
				break;
			}
...
static int ilitek_get_time_diff(void)
{
	...
	if (time_now.tv_usec < start_event_time.tv_usec) {
		...
		diff_milliseconds += (1000 * 1000 + time_now.tv_usec -
				      start_event_time.tv_usec) /
				     1000;
	} else
		diff_milliseconds +=
			(time_now.tv_usec - start_event_time.tv_usec) / 1000;

	if (diff_milliseconds < (-10000))
		diff_milliseconds = 10000;
	TP_MSG(NULL,
	       "time_now.tv_sec = %d start_event_time.tv_sec = %d time_now.tv_usec = %d start_event_time.tv_usec = %d diff_milliseconds = %d\n",
	       (int)time_now.tv_sec, (int)start_event_time.tv_sec,
	       (int)time_now.tv_usec, (int)start_event_time.tv_usec,
	       diff_milliseconds);
#else
	diff_milliseconds = jiffies_to_msecs(jiffies) -
			    jiffies_to_msecs(start_event_time_jiffies);
	TP_MSG(NULL,
	       "jiffies_to_msecs(jiffies) = %u jiffies_to_msecs(start_event_time_jiffies) = %u diff_milliseconds = %d\n",
...
static uint8_t ilitek_double_click_release(uint8_t finger_state)
{
	...
	if (finger_state == 3) {
		...
		if ((event_spacing < DOUBLE_CLICK_TOTAL_USED_TIME &&
		     event_spacing > 50) &&
		    (ABSSUB(current_x, start_x) < DOUBLE_CLICK_DISTANCE) &&
...
static void ilitek_report_touch_event(struct touch_data *touch, void *_private)
{
	...
	for (i = 0; i < touch->cnt; i++) {
		...
		touch->finger[i].x =
			(ILITEK_REVERT_X) ? x_max - touch->finger[i].x + x_min :
					    touch->finger[i].x;
		touch->finger[i].y =
			(ILITEK_REVERT_Y) ? y_max - touch->finger[i].y + y_min :
					    touch->finger[i].y;
...
static void ilitek_report_pen_event(struct pen_data *pen, void *_private)
{
	...
	tool = (pen->pen.in_range && pen->pen.invert) ? BTN_TOOL_RUBBER :
							BTN_TOOL_PEN;
	...
	pen->pen.x =
		(ILITEK_REVERT_X) ? x_max - pen->pen.x + x_min : pen->pen.x;
	pen->pen.y =
		(ILITEK_REVERT_Y) ? y_max - pen->pen.y + y_min : pen->pen.y;
...
static void ilitek_report_buf(uint8_t *buf, int size, bool is_last,
			      void *_private)
{
	UNUSED(is_last);
...
static int ilitek_request_irq(void)
{
	...
	mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM,
			     CUST_EINT_TOUCH_PANEL_TYPE, ilitek_i2c_isr, 1);
	mt_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
#else
...
static int ilitek_read_fw(char *filename, unsigned char *buf, int size,
			  void *data)
{
	int error, fw_size;
...
void ilitek_suspend(void)
{
	...
	if (ts->gesture_status) {
		...
		if (ts->low_power_status == Low_Power_Sleep) {
			mutex_lock(&ts->ilitek_mutex);
			if (api_protocol_set_cmd(ts->dev, SET_IC_SLEEP, NULL) <
			    0)
...
#if ILITEK_PLAT == ILITEK_PLAT_ALLWIN
	TP_MSG(NULL,
	       "(config_info.wakeup_gpio.gpio) = %d (config_info.int_number) = %d\n",
...
#ifdef CONFIG_OF
	if (ts->reset_gpio < 0)
		ts->reset_gpio = of_get_named_gpio(ts->device->of_node,
						   "ilitek,reset-gpio", 0);
	TP_ERR(NULL, "reset_gpio = %d\n", ts->reset_gpio);
	ts->irq_gpio =
		of_get_named_gpio(ts->device->of_node, "ilitek,irq-gpio", 0);
	if (ts->irq_gpio < 0)
...
#if defined(ILITEK_GPIO_DEBUG)
	do {
		ts->test_gpio = of_get_named_gpio(ts->device->of_node,
						  "ilitek,test-gpio", 0);
		if (ts->test_gpio < 0) {
...
static int ilitek_request_gpio(void)
{
	...
	if (ts->reset_gpio > 0) {
		...
		if (ret) {
			...
			if (ret)
				TP_ERR(NULL,
				       "Failed to direction output rest gpio err\n");
		}
	}
	if (ts->irq_gpio > 0) {
		...
		if (ret) {
			TP_ERR(NULL,
			       "Failed to request irq_gpio so free retry\n");
			gpio_free(ts->irq_gpio);
			ret = gpio_request(ts->irq_gpio, "ilitek-irq-gpio");
			if (ret)
...
static int __maybe_unused ilitek_alloc_dma(void)
{
	...
	I2CDMABuf_va = (u8 *)dma_alloc_coherent(&tpd->dev->dev, ILITEK_DMA_SIZE,
						&I2CDMABuf_pa, GFP_KERNEL);
	if (!I2CDMABuf_va) {
		TP_ERR(NULL,
		       "ilitek [TPD] tpd->dev->dev dma_alloc_coherent error\n");
		I2CDMABuf_va = (u8 *)dma_alloc_coherent(
			NULL, ILITEK_DMA_SIZE, &I2CDMABuf_pa, GFP_KERNEL);
		if (!I2CDMABuf_va) {
			TP_ERR(NULL,
...
#ifdef ILITEK_ENABLE_DMA
	if (I2CDMABuf_va) {
		dma_free_coherent(&tpd->dev->dev, ILITEK_DMA_SIZE, I2CDMABuf_va,
				  I2CDMABuf_pa);
...
static void __maybe_unused ilitek_release_regulator(void)
{
	...
	ILITEK_PLAT != ILITEK_PLAT_ALLWIN
	if (ts->vdd)
...
#ifdef ILITEK_WAKELOCK_SUPPORT
		wake_lock_init(&ilitek_wake_lock, WAKE_LOCK_SUSPEND,
			       "ilitek wakelock");
#endif
		return;
	}
...
int ilitek_main_probe(void *client, struct device *device)
{
	...
	res = of_property_read_u32(device->of_node, "ilitek_change_x2y",
				       &ts->change_x2y);
 
	if (res < 0) {
		printk("of_property_read_u32 ilitek_change_x2y failed\n");
		// return res;
	}
	...
	if (!ts->dev){
		printk("No dev , go to free  failed\n"); 
		goto err_free_gpio;
	}
